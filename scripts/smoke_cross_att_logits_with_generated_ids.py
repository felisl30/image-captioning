...
def main() -> None:
    ...
    print("Extrayendo QK logits con generated_ids...")
    ...
    for word, heatmap in maps[:10]:
        print(
            ...
            f"min={float(heatmap.min()):.4f} max={float(heatmap.max()):.4f}"
        )
    ...
    out_path = out_dir / f"smoke_qk_logits_base_idx_{idx}.npz"
    torch_ids_cpu = generated_ids.detach().cpu().numpy()

    import numpy as np

    np.savez_compressed(
        out_path,
        generated_ids=torch_ids_cpu,
    ...
    print("OK cross_att_logits acepta generated_ids.")
